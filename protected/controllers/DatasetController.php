    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('view', 'checkDOIExist', 'aSetSortCookies', 'next', 'resetPageSize'),
                'users' => array('*'),
            ),
...
EO_MAIL;
    ...
    public function actionNext() {
        if (isset($_GET['doi'])) {
            ...
            $result = Dataset::model()->findAllBySql("select identifier from dataset where identifier > '" . $doi . "' and upload_status='Published' order by identifier asc limit 1;");
            if (count($result) == 0) {
                $result = Dataset::model()->findAllBySql("select identifier from dataset where upload_status='Published' order by identifier asc limit 1;");
                $next_doi = $result[0]->identifier;
            } else {
